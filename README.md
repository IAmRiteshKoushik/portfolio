...
This site now supports blog comments via GitHub Discussions using `giscus`.
...
4. Copy `.env.example` to `.env` and fill in the `PUBLIC_GISCUS_*` values generated by `https://giscus.app/en`.

Each blog post maps to its own discussion thread using the post pathname.
